...
## Repository polish pass after the completed two-model pilot (2026-08-21)

By this point, both the Qwen pilot (`docs/qwen_pilot_results.md`) and the
Llama replication had actually been run and frozen, and their two-model
synthesis was committed in `docs/cross_model_pilot_results.md` (commit
`d9d2cb7`) with a matching README update (commit `49003b5`). This entry
records a subsequent, separate polish pass: no experiment was rerun, no
frozen numeric result was changed, and no historical pre-run document was
rewritten to look as if it had been written after the fact.

**Stale current-status text fixed.** `data/README.md`, `results/README.md`,
and `figures/README.md` still said things like "no real pilot has been
run" and "prepare-data has not been run in this environment" — true when
those files were written, no longer true once both research-model runs
completed. They were updated to state plainly that both runs happened (on
the researcher's own GPU host), that the large runtime artifacts those
runs produced are intentionally gitignored (so this checkout's
`data/`/`results/`/`figures/` directories are legitimately empty), and to
point readers at the frozen result documents. Historical/frozen documents
were deliberately left alone: `docs/qwen_pilot_results.md`,
`docs/cross_model_pilot_results.md`, `docs/phase2_research_design.md`,
`docs/llama_replication_protocol.md`, and the pre-run
`configs/replication/*.yaml` templates all still describe, correctly,
what was true or planned at the time they were written; none of their
"pre-run"/"not yet run" wording was replaced.

**`configs/frozen/` added** as post-run provenance copies
(`qwen_pilot_frozen.yaml`, `qwen_model_frozen.yaml`,
`llama_pilot_frozen.yaml`, `llama_model_frozen.yaml`, plus a README).
Every field in these files is either copied unchanged from the
corresponding pre-run template or filled in from a value the frozen
result documents themselves already report (resolved model revisions,
confirmed source roles, and, for Qwen only, the machine-specific
`max_memory` cap `docs/qwen_pilot_results.md` records for that run). No
value was invented: Llama's `max_memory` and per-run software-environment
versions are not recorded anywhere in `docs/cross_model_pilot_results.md`,
so those fields stay unset rather than being assumed equal to Qwen's. The
original pre-run templates (`configs/pilot.yaml`,
`configs/models.yaml`, `configs/replication/llama_pilot.yaml`,
`configs/replication/models_llama.yaml`) are untouched and remain the
historical record of what was precommitted before either run happened.

**Curated cross-model summary figures added** under `docs/assets/`
(`cross_model_harmful_vs_corrective.png`, `cross_model_margin_strength.png`),
generated by `scripts/plot_cross_model_summary.py`. No raw per-item
result files exist in this checkout to plot from (they are gitignored
runtime artifacts), so the script visualizes only already-frozen aggregate
numbers copied verbatim from `docs/cross_model_pilot_results.md`
(Sections 4 and 7), with each constant commented against its source line
and the two models' margin-bin panels kept visually and textually separate
because they use different pooling definitions (Qwen: preferred-source
only, n=10/bin; Llama: pooled across both sources, n=20/bin) that are not
directly comparable.

**Not done in this pass.** No GitHub CLI/API access was available in this
environment, so the latest CI run on `main` could not be observed; no CI
badge was added (a badge was previously deferred for the same reason — see
`README.md`, "Continuous integration"). No `LICENSE` file was added; the
choice remains a pending researcher decision (see "Repository license —
